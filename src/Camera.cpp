...
Camera::Camera(Device* device, float aspectRatio) : device(device) {
	...
	glm::mat4 invTransform = glm::inverse(cameraBufferObject.viewMatrix);
    cameraBufferObject.viewDir = glm::vec4(glm::normalize(glm::vec3(-invTransform[2])), 0.0f);
	cameraBufferObject.viewPos = glm::vec4(glm::vec3(invTransform[3]), 1.0f);
...
void Camera::UpdateOrbit(float deltaX, float deltaY, float deltaZ) {
    ...
    cameraBufferObject.viewDir = glm::vec4(glm::normalize(glm::vec3(-finalTransform[2])), 0.0f);
	cameraBufferObject.viewPos = glm::vec4(glm::vec3(finalTransform[3]), 1.0f);
